    ...
    if (orderData.status === OrderStatus.PAID) {
      ...
      await emailService.sendOrderEmail(
        ...
        `Order Confirmation - #${orderData.orderNumber}`,
        {
          ...
          items: orderData.items.map((i: { product: { name: string }; packSize: string | null; quantity: number; price: number }) => ({
        ...
        await emailService.sendAdminEmail(
          `New Paid Order Placed - #${orderData.orderNumber}`,
          {
            ...
            details: [
              ...
              `Items: ${orderData.items.map((i: { product: { name: string }; quantity: number }) => `${i.product.name} (Qty: ${i.quantity})`).join(', ')}`
            ],
